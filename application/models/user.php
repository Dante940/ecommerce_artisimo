...
class User extends CI_Model {
	...
	public function register_user($post) {
		...
		if($this->form_validation->run() === FALSE) {
			...
			$query = "INSERT INTO users (first_name, last_name, email, password, created_at, updated_at, admin) VALUES (?, ?, ?, ?, NOW(), NOW(), ?)";
			$values = array($post['first_name'], $post['last_name'], $post['email'], $post['password'], 0);
